# 📊 Projeto de Gerência de Software – Plasti&Cia

![Status](https://img.shields.io/badge/status-em%20desenvolvimento-yellow)
![Curso](https://img.shields.io/badge/curso-Análise%20e%20Desenvolvimento%20de%20Sistemas-blue)
![Instituição](https://img.shields.io/badge/Instituição-UNISUAM-red)
![Ano](https://img.shields.io/badge/Ano-2026-green)
![Tipo](https://img.shields.io/badge/Projeto-Acadêmico-lightgrey)

---
##AVISO
## O TRABALHO DO GRUPO SE ENCONTRA NO PULL REQUESTS, NO ÚLTIMO ARQUIVO ENVIADO. 

## 📌 Resumo
Este projeto tem como objetivo analisar os processos da empresa **Plasti&Cia** e propor melhorias por meio da implementação de um sistema ERP, visando otimizar a gestão, comunicação e controle operacional.

---

## 🏢 Sobre a Empresa
- Fundada em 2005  
- Localização: Rio de Janeiro  
- ~10 lojas  
- ~200 funcionários  
- Atuação: varejo e atacado de materiais plásticos, domésticos e tecidos  

---

## ⚠️ Principais Problemas
- Falhas na comunicação interna  
- Processos não padronizados  
- Sobrecarga na gestão de equipes  
- Controle de estoque ineficiente  
- Dependência de conhecimento informal  

---

## 💡 Solução Proposta
Implementação de um **sistema ERP** para:
- Integrar todas as lojas  
- Centralizar informações  
- Melhorar controle de estoque  
- Apoiar decisões estratégicas  

---

## 👥 Equipe
- Jefferson Oliveira  
- Maiara Nicolau  
- Matheus Kauan  
- Raphael Freitas  

---

## 🎓 Informações Acadêmicas
- Curso: Análise e Desenvolvimento de Sistemas  
- Instituição: UNISUAM  
- Professor Orientador: Kellen Nery  

---

## 📍 Local
Rio de Janeiro, RJ – 2026
